...
def get_metrics():
    ...
    try:
        ...
        current_app.logger.error(f"Failed to get dashboard metrics: {str(e)}")
...
def get_alerts():
    ...
    try:
        ...
        current_app.logger.error(f"Failed to get dashboard alerts: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def get_backups():
    ...
    try:
        ...
        current_app.logger.error(f"Failed to get backup information: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def get_performance():
    ...
    try:
        ...
        performance = {
            'system': {
                'timestamps': [m.timestamp.isoformat() for m in system_history],
                'cpu': [m.cpu_percent for m in system_history],
                'memory': [m.memory_percent for m in system_history]
            },
            'api': {
                'timestamps': [m.timestamp.isoformat() for m in api_monitor.metrics_history],
                'response_times': {
                    provider: [m['avg_response_time'] for m in metrics['history']]
                    for provider, metrics in api_monitor.collect_metrics().items()
                }
            }
        }
        ...
        current_app.logger.error(f"Failed to get performance metrics: {str(e)}")
        return jsonify({'error': str(e)}), 500
